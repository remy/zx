...
(function(scope) {
    ...
    var onmessage = function(e) {
      ...
      switch (msg.type) {
        ...
        case 'state':
          ...
          if (node) {
            if (msg.state == 'running') node.processor = msg.processor;
  ...
  AWPF.AudioWorkletNode = function(context, nodeName, options) {
    ...
    function configureBus(type, options) {
      var numPorts =
        type == 'input' ? options.numberOfInputs : options.numberOfOutputs;
      var channelCount =
        type == 'input'
    ...
    else args.bufferCount = bufferCount;
    AWPF.worker.postMessage({ type: 'createProcessor', args: args }, [
      messageChannel.port2,
    ]);

    this.onprocessorstatechange = function(e) {
      this.processorState = e.detail;
      if (!AWPF.hasSAB && this.processorState == 'running') render();
    ...
    if (AWPF.hasSAB) var outbuf = new Float32Array(this.outputBus[0][0]);
    // spn limitation
    ...
    var onprocess = function(ape) {
      ...
      if (AWPF.hasSAB) {
        ...
        if (outbuf.byteLength) outL.set(outbuf);
        if (newBufferAvailable) render();
      }
    };
  };
  ...
  AWPF.polyfill = function(scope) {
    return new Promise(function(resolve) {
      if (AWPF.AudioWorkletAvailable(scope)) resolve();
      else {
        ...
        console.warn(
          'Using Worker polyfill of AudioWorklet, audio will not be performance isolated.'
        );
